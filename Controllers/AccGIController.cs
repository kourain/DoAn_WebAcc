...
namespace DoAn_WebAcc.Controllers
{
    public class AccGIController : Controller
    {
        ...
        public IActionResult Search([FromQuery] string? price, [FromQuery] string sort_price, [FromQuery] string? acc_server, [FromQuery] int? page)
        {
            ...
            var acc = _dataContext.AccGIs.Where(p => (p.Sold == null || p.Sold == 0) 
                    &&p.Price>min_price 
                    && p.Price<max_price );
            if (sort_price == "desc")
            {
        ...
        public ActionResult Buy(AccGI acc)
        {
            var accbuy = _dataContext.AccGIs.Where(m => m.UID == acc.UID && (m.Sold == null || m.Sold == 0)).FirstOrDefault();
            if (accbuy != null)
            {
                accbuy.SoldDate = DateTime.Now;
                accbuy.Sold = Functions._UserID;
                _dataContext.AccGIs.Update(accbuy);
                _dataContext.SaveChanges();
                return RedirectToAction("HistoryBuy");
            }
            return RedirectToAction("AccGI_Acc", acc.UID);
        }
        [Route("/AccGI/HistoryBuy")]
        public IActionResult HistoryBuy()
        {
            if (!Functions.IsLogin())
            {
                return Redirect("/Login?returnUrl=/AccGI/HistoryBuy");
            }
            var acc = _dataContext.AccGIs.Where(m => (m.Sold == Functions._UserID)).ToList();
            return View(acc);
        }
